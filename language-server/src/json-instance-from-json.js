...
export class JsoncInstanceFromJSON {
  constructor(json, root, node, pointer, annotations) {
    this.json = json;
    this.root = root;
    this.node = node;
  ...
  static fromJSON(json) {
    ...
    return new JsoncInstanceFromJSON(json, root, root, "", {});
  }
  ...
  *iter() {
    ...
    for (let itemIndex = 0; this.node.children[itemIndex]; itemIndex++) {
      ...
      yield new JsoncInstanceFromJSON(
        this.json,
        this.root,
        itemNode,
        pointer,
  ...
  *values() {
  ...
  get(uri = "") {
    ...
    let result = new JsoncInstanceFromJSON(
      this.json,
      this.root,
      this.root,
      "",
    ...
    for (const segment of pointerSegments(pointer)) {
      ...
      if (result.typeOf() === "object") {
        const pair = find(
          ([propertyName]) => propertyName.value() === segment,
          result.entries()
        );
        result = pair?.[1];
  ...
  asEmbedded() {
    return new JsoncInstanceFromJSON(this.json, this.node, this.node, "", {});
  }
